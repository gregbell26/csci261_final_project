...
THS::Texture::Texture() :
    m_data(nullptr), m_textSize(new THS::Size(0,0)), m_renderLocation(new THS::Size(0,0))
{}
...
THS::Texture::Texture(const Texture& SRC) :
    m_data(nullptr), m_textSize(new THS::Size(SRC.m_textSize->height, SRC.m_textSize->width)), 
    m_renderLocation(new THS::Size(SRC.m_renderLocation->height, SRC.m_renderLocation->width))
{
...
THS::Texture& THS::Texture::operator=(const THS::Texture& SRC) {
    // because this is completely reassigning it - we can delete the old object
    this->~Texture();

    //deep copy time!

    m_textSize = new THS::Size(SRC.m_textSize->height, SRC.m_textSize->width);
    m_renderLocation= new THS::Size(SRC.m_renderLocation->height, SRC.m_renderLocation->width);
